...
extension FlashcardsViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let flashcards = deck?.flashcards?.allObjects as? [Flashcard] {
            ...
            let tierPrefix = flashcard.tier > 0 ? "[T\(flashcard.tier)] " : ""
            let stateIcon: String
            switch flashcard.effectiveCardState {
            case "locked": stateIcon = "🔒 "
            case "mastered": stateIcon = "✅ "
            case "learning": stateIcon = ""
            default: stateIcon = ""
            }
            cell.frontLabel.text = "\(stateIcon)\(tierPrefix)\(flashcard.frontLabel ?? "")"
            let bullets = flashcard.bulletPoints
            if !bullets.isEmpty {
                cell.backLabel.text = bullets.map { "• \($0)" }.joined(separator: "\n")
            } else {
